...
pub trait CursorState: Sized + Copy + Clone {
    ...
    fn get_cfa<R, S>(&self, row: &gimli::UnwindTableRow<R, S>, g_ctx: &GlobalContext) -> Result<usize, UnwindError>
        where
            R: gimli::Reader,
            S: UnwindContextStorage<R>;

    fn step<R, S>(&mut self, row: &gimli::UnwindTableRow<R, S>, g_ctx: &GlobalContext) -> Result<(), UnwindError>
        where
            R: gimli::Reader,
            S: UnwindContextStorage<R>;

    fn recover_register<R, S>(&self, reg: Register, row: &gimli::UnwindTableRow<R, S>, g_ctx: &GlobalContext) -> Result<usize, UnwindError>
        where
            R: gimli::Reader,
            S: UnwindContextStorage<R> {
        self.get_cfa(row, g_ctx).and_then(|cfa| {
            match row.register(reg) {
                ...
                RegisterRule::ValOffset(offset) => {
                    Ok((cfa as i64 + offset) as usize)
                }
                RegisterRule::Register(target) => self.get_register(target),
                RegisterRule::Expression(_) => { todo!() }
                RegisterRule::ValExpression(_) => { todo!() }
                RegisterRule::Architectural => Err(UnwindError::NotSupported("target register recovery is architectural"))
            }
        })
    }
}
