...
impl CursorState for FramePointerBasedState {
    ...
    fn get_cfa<R, S>(&self, row: &UnwindTableRow<R, S>, _: &GlobalContext) -> Result<usize, UnwindError>
        where
            R: Reader,
        ...
        match row.cfa() {
            CfaRule::RegisterAndOffset { register, offset } => {
                if register.0 == STACK_POINTER_IDX {
                    ...
                    Err(UnwindError::NotSupported("CFA value is only derivable from RSP in frame pointer based state"))
                }
            }
            CfaRule::Expression(_) => {
                Err(UnwindError::NotSupported("CFA expression is not supported in frame pointer based state"))
            }
        }
    }

    fn step<R, S>(&mut self, row: &UnwindTableRow<R, S>, g_ctx: &GlobalContext) -> Result<(), UnwindError> where R: Reader, S: UnwindContextStorage<R> {
